...
// "allowHatInHelmetSlot" sous "COMMON". Un fichier déposé à plat
// ({ "allowHatInHelmetSlot": true }) n'est pas reconnu par owo, qui retombe
// alors sur le défaut "false" → écran "unrecoverable config mismatch" à la
// connexion serveur (l'option est synchronisée mais exige un redémarrage client).
//
...
//                              par le serveur) → aucun mismatch collatéral.
...
// Fusion profonde de `patch` dans `base` (objets simples ; les tableaux et
// valeurs scalaires du patch remplacent celles de la base).
