...
contextBridge.exposeInMainWorld("launcher", {
	...
	onChatChannels: (cb) => {
		const h = (_, d) => cb(d);
		ipcRenderer.on("chat:channels", h);
		return () => ipcRenderer.removeListener("chat:channels", h);
	},
	onChatMessages: (cb) => {
		const h = (_, d) => cb(d);
		ipcRenderer.on("chat:messages", h);
		return () => ipcRenderer.removeListener("chat:messages", h);
	}
});
//#endregion
